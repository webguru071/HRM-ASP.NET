...
namespace EMSApp.Controllers
{
    public class SellsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                var data = db.INV_INFO.Where(x => x.STOCK_TYPE == ConstantValue.TransactionSheetTransTypeSell).ToList();
                return View(data);
            }
            else
            {
        ...
        public ActionResult Create(INV_INFO collection)
        {
            try
            {
                ...
                else
                {
                    ...
                    stock.STOCK_TYPE = collection.STOCK_TYPE = ConstantValue.TransactionSheetTransTypeSell;
        ...
        public ActionResult Edit(int id)
        {
            if (converterHelper.CheckLogin() && converterHelper.GetLoggedUserLevel() == ConstantValue.UserLevelAdmin)
            {
                var data = db.INV_INFO.Where(x => x.INV_ID == id && x.STOCK_TYPE == ConstantValue.TransactionSheetTransTypeSell).FirstOrDefault();
        ...
        {
            List<SelectListItem> list = new SelectList(db.VENDOR_INFO.Where(x => x.VENDOR_TYPE == ConstantValue.VendorTypeBuyer), "VENDOR_ID", "VENDOR_NAME").ToList();
            list.Insert(0, (new SelectListItem { Text = "Select One", Value = "0" }));
            return list;
        }
    }
